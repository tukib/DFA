export default class DFA {
    ...
    */
    ...
    addAcceptingState(state) {
        ...
        if (this.acceptingStates.includes(state)) throw new Error('State is already an accepting state');
    ...
    runUntilCompletion(input) {
        // TODO: fix bug that hangs this on empty input
    ...
    loadTransitionList(list, transitionCallback) {
        // s1, a, s2; s1, b, s3; ...
        if (list.length === 0) throw new Error('Empty transition list');
        const transitions = list.split(';');
        // first check if valid
        for (let i in transitions) {
            transitions[i] = transitions[i].split(',').map(str => str.trim());
            if (transitions[i].length !== 3 || !transitions[i][0] || !transitions[i][1] || !transitions[i][2]) {
                throw new Error(`Malformed transition list (at: ${transitions[i]})`);
            }
        }
